...
class PoneglyphRules:
    ...
    def actualizar_todos_con_lagrange(self, turno_actual, partida_id=None):
        total = 0
        ...
        return total
    def actualizar_precio_con_lagrange(self, poneglyph, turno_actual=1, partida_id=None):
        try:
    ...
    def analizar_poneglyph(self, jugador, casilla, turno_actual=1, partida_id=None):
        poneglyph = self.obtener_poneglyph(casilla.referencia)
        
        if poneglyph is not None:
            self.actualizar_precio_con_lagrange(
            poneglyph,
            turno_actual,
            partida_id
        )
