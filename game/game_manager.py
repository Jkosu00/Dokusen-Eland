...
from database.db import (
    ...
    guardar_transaccion,
    guardar_propiedad,
    actualizar_jugador_estado,
    actualizar_propiedad_estado,
    guardar_evento,
    guardar_yonkou,
    actualizar_ganador
)
...
class GameManager:
    def __init__(
        ...
        self._iniciar_registro_partida()
    ...
    def _iniciar_registro_partida(self):
        """
        Crea una nueva partida en la base de datos y registra el estado inicial de jugadores y propiedades.
        """

        inicializar_base_datos()

        self.partida_id = guardar_partida(
            cantidad_jugadores=len(self.jugadores)
        )

        print(f"Partida registrada en BD con ID: {self.partida_id}")

        for jugador in self.jugadores:
            guardar_jugador(
                partida_id=self.partida_id,
                nombre=jugador.nombre,
                berries=jugador.dinero
            )
        ...
        for propiedad in self.cell_resolver.property_rules.propiedades.values():
            guardar_propiedad(
                ...
                tipo=propiedad.tipo,
                grupo_color=propiedad.grupo,
                propietario=None,
                nivel_mejora=propiedad.nivel_mejora,
                precio_actual=propiedad.precio_actual
            )

        for poneglyph in self.cell_resolver.poneglyph_rules.poneglyphs.values():
            guardar_propiedad(
                partida_id=self.partida_id,
                nombre=poneglyph.nombre,
                tipo=poneglyph.tipo,
                grupo_color=poneglyph.grupo,
                propietario=None,
                nivel_mejora=poneglyph.nivel_mejora,
                precio_actual=poneglyph.precio_actual
            )
        self._sincronizar_estado_bd()

    def _sincronizar_estado_bd(self):
        """
        Actualiza en la base de datos el estado actual de jugadores y propiedades de la partida.
        """
        ...
        for jugador in self.jugadores:
            actualizar_jugador_estado(
                ...
                nombre=jugador.nombre,
                berries=jugador.dinero,
                riqueza_total=jugador.dinero + self._calcular_valor_total_venta(jugador),
                monopolios=self._contar_monopolios(jugador),
                road_poneglyphs=jugador.cantidad_road_poneglyphs()
            )
        ...
        for propiedad in self.cell_resolver.property_rules.propiedades.values():
            actualizar_propiedad_estado(
                partida_id=self.partida_id,
                nombre=propiedad.nombre,
                propietario=propiedad.dueno.nombre if propiedad.dueno else None,
                nivel_mejora=propiedad.nivel_mejora,
                precio_actual=propiedad.precio_actual
            )
        ...
        for poneglyph in self.cell_resolver.poneglyph_rules.poneglyphs.values():
            actualizar_propiedad_estado(
                partida_id=self.partida_id,
                nombre=poneglyph.nombre,
                propietario=poneglyph.dueno.nombre if poneglyph.dueno else None,
                nivel_mejora=poneglyph.nivel_mejora,
                precio_actual=poneglyph.precio_actual
            )
    
    def _registrar_transaccion_bd(
        self,
        jugador,
        tipo,
        monto,
        ...
        """
        Guarda una transacción económica de la partida. monto positivo = ingreso. monto negativo = egreso.
        """
        ...
        guardar_transaccion(
            ...
            tipo=tipo,
            monto=monto,
            propiedad=propiedad.nombre if propiedad else None,
            descripcion=descripcion
        )
    

    def _registrar_evento_bd(self, jugador, tipo_evento, efecto, monto=0):
        """
        Guarda eventos importantes de la partida.
        No necesariamente implican dinero.
        """
    ...
    def _manejar_accion_prision(self, jugador, accion):
        if accion == "pagar_prision":
            ...
            self._registrar_transaccion_bd(
                jugador=jugador,
                tipo="PAGO_PRISION",
                monto=-self.costo_salida_prision,
                descripcion="Pagó para salir de prisión."
            )
            self._sincronizar_estado_bd()
    ...
    def _iniciar_movimiento_con_dados(self, jugador):
        if self.board_manager.paso_por_salida(jugador.posicion, self.suma_dados):
            ...
            self._registrar_transaccion_bd(
                ...
                tipo="BONO_SALIDA",
                monto=bono,
                descripcion="Bono por pasar por Salida calculado con Lagrange."
            )

            self._registrar_evento_bd(
                jugador=jugador,
                tipo_evento="salida",
                efecto="Pasó por Salida y recibió bono por Lagrange.",
                monto=bono
            )
            self._sincronizar_estado_bd()
    ...
    def _manejar_casilla_descanso(self, jugador):
        ...
        self._registrar_transaccion_bd(
            ...
            tipo="BONO_DESCANSO",
            monto=bono,
            descripcion="Bono por casilla Descanso calculado con Lagrange."
        )
        self._registrar_evento_bd(
            jugador=jugador,
            tipo_evento="descanso",
            efecto="Recibió bono de descanso calculado con Lagrange.",
            monto=bono
        )
    ...
    def _manejar_casilla_prision(self, jugador):
        ...
        self._registrar_evento_bd(
            jugador=jugador,
            tipo_evento="PRISION",
            efecto=f"{jugador.nombre} fue enviado a prisión.",
            monto=0
        )
    ...
    def _manejar_casilla_evento(self, jugador, casilla):
        if casilla.referencia == "evento_riesgo":
            self._registrar_evento_bd(
                jugador=jugador,
                tipo_evento="casilla_evento_riesgo",
                efecto=f"{jugador.nombre} cayó en una casilla de Evento Riesgo.",
                monto=0
            )
    ...
    def _resolver_evento_recompensa(self, jugador):
        ...
        self._registrar_transaccion_bd(
            jugador=jugador,
            tipo="COFRE_RECOMPENSA",
            monto=cantidad,
            descripcion="Recompensa de cofre calculada con Lagrange."
        )
        self._registrar_evento_bd(
            jugador=jugador,
            tipo_evento="COFRE_RECOMPENSA",
            efecto=f"{jugador.nombre} abrió un cofre y recibió ${cantidad} Berries.",
            monto=cantidad
        )
        self._sincronizar_estado_bd()
    ...
    def _resolver_riesgo_malo(self, jugador):
        ...
        if evento == "perder_dinero":
            ...
            self._registrar_transaccion_bd(
                jugador=jugador,
                tipo="EVENTO_PIERDE_DINERO",
                monto=-cantidad,
                descripcion="Evento de riesgo: pierde dinero."
            )
        ...
        self._registrar_evento_bd(
            jugador=jugador,
            tipo_evento="RIESGO_PIERDE_PROPIEDAD",
            efecto=f"{jugador.nombre} perdió {nombre_bien}. La propiedad quedó libre.",
            monto=0
        )
    ...
    def _resolver_riesgo_bueno(self, jugador):
        ...
        if evento == "ganar_dinero":
            ...
            self._registrar_transaccion_bd(
                jugador=jugador,
                tipo="EVENTO_GANA_DINERO",
                monto=cantidad,
                descripcion="Evento favorable: gana dinero calculado con Lagrange."
            )
        ...
        if evento == "conseguir_yonkou":
            ...
            if not propiedades:
                self._registrar_evento_bd(
                    jugador=jugador,
                    tipo_evento="YONKOU_SIN_ISLAS",
                    efecto=f"{jugador.nombre} obtuvo influencia Yonkou, pero no tenía islas propias.",
                    monto=0
                )
            ...
            self._registrar_evento_bd(
                jugador=jugador,
                tipo_evento="CONSIGUE_YONKOU",
                efecto=f"{jugador.nombre} obtuvo influencia Yonkou y puede aplicarla a una isla.",
                monto=0
            )
    ...
    def _resolver_click_compra_isla(self, jugador, propiedad, accion_modal):
        ...
        if nivel_objetivo is None:
            return
        precio_total = self._calcular_precio_compra_con_nivel(
            propiedad,
            nivel_objetivo
        )
        ...
        self._registrar_transaccion_bd(
            ...
            monto=-precio_total,
            propiedad=propiedad,
            descripcion=f"Compra de {propiedad.nombre} en nivel {nivel_objetivo}."
        )
    ...
    def _resolver_click_mejora(self, jugador, propiedad, accion_modal):
        ...
        self._registrar_transaccion_bd(
            ...
            monto=-costo,
            propiedad=propiedad,
            descripcion=f"Mejora de {propiedad.nombre}."
        )
    ...
    def _resolver_click_compra_poneglyph(self, jugador, propiedad, accion_modal):
        ...
        self._registrar_transaccion_bd(
            jugador=jugador,
            tipo="COMPRA_PONEGLYPH",
            monto=-precio,
            propiedad=propiedad,
            descripcion=f"Compra de {propiedad.nombre}."
        )
    ...
    def _resolver_click_recompra(self, jugador, propiedad, accion_modal):
        ...
        if accion_modal == "pagar_impuesto":
            ...
            self._registrar_transaccion_bd(
                ...
                monto=-impuesto,
                propiedad=propiedad,
                descripcion=f"Pagó impuesto por caer en {propiedad.nombre}."
            )
            ...
            if dueno:
                self._registrar_transaccion_bd(
                    ...
                    monto=impuesto,
                    propiedad=propiedad,
                    descripcion=f"Cobró impuesto de {propiedad.nombre}."
                )
        ...
        if accion_modal == "pagar_y_recomprar":
            ...
            self._registrar_transaccion_bd(
                ...
                monto=-impuesto,
                propiedad=propiedad,
                descripcion=f"Pagó impuesto antes de recomprar {propiedad.nombre}."
            )
            ...
            self._registrar_transaccion_bd(
                ...
                monto=-recompra,
                propiedad=propiedad,
                descripcion=f"Recompró {propiedad.nombre}."
            )

            if dueno_anterior:
                self._registrar_transaccion_bd(
                    jugador=dueno_anterior,
                    tipo="COBRO_IMPUESTO",
                    monto=impuesto,
                    propiedad=propiedad,
                    descripcion=f"Cobró impuesto antes de perder {propiedad.nombre}."
                )
                ...
                self._registrar_transaccion_bd(
                    ...
                    monto=recompra,
                    propiedad=propiedad,
                    descripcion=f"Recibió pago por recompra de {propiedad.nombre}."
                )
            ...
            if rect.collidepoint(pos_mouse) and casilla.referencia in referencias:
                ...
                if propiedad:
                    ...
                    guardar_yonkou(
                        partida_id=self.partida_id,
                        turno=self.turn_manager.turno_numero,
                        jugador_yonkou=jugador_actual.nombre,
                        isla_mejorada=propiedad.nombre
                    )

                    self._registrar_evento_bd(
                        jugador=jugador_actual,
                        tipo_evento="APLICA_YONKOU",
                        efecto=f"{jugador_actual.nombre} aplicó Bounty Yonkou a {propiedad.nombre}.",
                        monto=0
                    )
            ...
            if rect.collidepoint(pos_mouse) and casilla.referencia in referencias:
                ...
                if propiedad:
                    ...
                    self._registrar_evento_bd(
                        ...
                        tipo_evento="DESTRUYE_PROPIEDAD",
                        efecto=f"{nombre}, propiedad de {dueno}, fue destruida y quedó libre.",
                        monto=0
                    )
    ...
    def _declarar_victoria(self, jugador, motivo):
        ...
        actualizar_ganador(
            ...
            condicion_victoria=motivo
        )

        self._registrar_evento_bd(
            jugador=jugador,
            tipo_evento="VICTORIA",
            efecto=f"{jugador.nombre} ganó la partida: {motivo}.",
            monto=0
        )
    ...
    def _verificar_bancarrota_actual(self):
        ...
        self._registrar_evento_bd(
            jugador=jugador,
            tipo_evento="BANCARROTA",
            efecto=f"{jugador.nombre} quedó con saldo negativo y debe vender propiedades.",
    ...
    def _manejar_click_venta_bancarrota(self, pos_mouse):
        ...
        for casilla in self.board_manager.obtener_todas_las_casillas():
            ...
            if rect.collidepoint(pos_mouse) and casilla.referencia in referencias:
                ...
                self._registrar_transaccion_bd(
                    jugador=jugador,
                    tipo="VENTA_BANCARROTA",
                    monto=valor,
                    propiedad=bien,
                    descripcion=f"Vendió {nombre_bien} por bancarrota."
                )
    ...
    def _eliminar_jugador_por_bancarrota(self, jugador):
        ...
        self._registrar_evento_bd(
            jugador=jugador,
            tipo_evento="ELIMINADO_BANCARROTA",
            efecto=f"{jugador.nombre} fue eliminado por bancarrota.",
            monto=jugador.dinero
        )
